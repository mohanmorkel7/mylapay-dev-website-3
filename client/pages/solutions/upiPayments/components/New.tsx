...
const rotatingContent = [
  {
    ...
    points: [
      "Enabling PSP and Beneficiary switch for financial institution and payment aggregators, Operating in merchant acquiring program and UPI payment gateways",
    ],
  },
  {
    image: image2,
    title: "Financial Institutions (PA-O / PA-CB)",
    icon: <CreditCard className="w-6 h-6 text-white" />,
    points: ["Operating in payment aggregation Offering payment gateway to merchants"],
  },
  {
    image: image3,
    title: "Point of Sale Providers (PA-P)",
    ...
    <div className="bg-[#1f2b5c] text-white py-16 px-4 md:px-16">
      ...
      >
        ...
        <p className="mt-6 text-center text-white max-w-2xl" style={{ fontFamily: "'Surt-Normal-TRIAL', sans-serif", fontWeight: 400, fontSize: "24px", lineHeight: "37px" }}>
          designed to deliver seamless end-to-end UPI payment solution for providers
        </p>
      </motion.div>
...
{!holdAll ? (
  ...
  >
    {rotatingContent.map((item, idx) => (
      ...
      >
        {[
          { title: "UPI Switch", desc: "PSP and beneficiary switches to streamline acquiring payment flows." },
          { title: "Real-Time Processing", desc: "Handles reconciliation, settlement, adjustments, and chargeback workflows instantly." },
          { title: "Versatile Use Cases", desc: "Supports both UPI Pay and UPI Collect transactions." },
          { title: "Seamless Connectivity", desc: "Fully integrated with the NPCI EAS network for uninterrupted operations." },
        ].map((item, i) => (
          <motion.div
            key={i}
            whileHover={{
            ...
            whileTap={{ scale: 0.98 }}
            className="border border-blue-400 rounded-lg p-6 text-center bg-white/10 backdrop-blur-lg shadow-md cursor-default"
          >
